...
export default function Payments() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.invoice || formData.invoice.toString().trim() === '') {
    ...
    try {
      const submitData = {
        ...formData,
        invoice: formData.invoice.toString().trim(),
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Total Invoice Amount</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {payments.length === 0 ? (
              ...
              payments.map((payment) => {
                ...
                const totalInvoiceAmount = Number(payment.totalInvoiceAmount) ?? payment.invoice?.totalAmount ?? 0;
                const amountPaid = Number(payment.amountPaid) ?? payment.amount ?? 0;
                const amountRemaining = Number(payment.amountRemaining) ?? 0;
                return (
                  <tr key={paymentId}>
